...
本章的工程与商业原则，可通过[客户验证](../practice/customer-discovery.md)、[可持续经营](../practice/sustainable-business.md)、[AI 工作流](../practice/ai-workflows.md)和[可靠性评测](../practice/ai-evaluation.md)进一步落到一次小实验。
...
随着工具调用和智能体编排变得普遍，资源层还要处理互操作与授权边界。[MCP 规范](https://modelcontextprotocol.io/specification/2025-06-18)把资源、提示和工具定义为可组合的连接能力，但不会替系统完成安全审查；接入任何外部服务器前，仍需列出数据范围、写入权限、用户同意、调用记录和撤回路径。协议降低的是重复接入成本，不是责任成本。实践上可用 [AI 工作流](../practice/ai-workflows.md) 的授权门和 [AI 评测](../practice/ai-evaluation.md) 的过程/结果记录先做小范围验证。
